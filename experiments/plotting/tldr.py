...
def plot_results(res, save=False):
    ...
    for method, name, marker, c, coords, _ in zip(
        ...
        print(method, mean.mean(), mean.std())
    ...
    plt.tight_layout()
    if save:
        plt.savefig(f"{BASE_PATH}/results/figures/tldr.png", bbox_inches="tight")
    # plt.show()
...
if args.use_wandb:
    runs = api.runs(f"{wandb_entity}/tldr_seeded")
    ...
    if args.save_res_local:
        with open(f"{BASE_PATH}/results/numerics/tldr.pkl", "wb") as f:
            pickle.dump((res, values), f)

else:
    try:
        with open(f"{BASE_PATH}/results/numerics/tldr.pkl", "rb") as f:
